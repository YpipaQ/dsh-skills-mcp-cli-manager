## =============================================================================
# @zebbkira/dsh-skills-mcp-manager — 一键安装脚本（Windows PowerShell 5.1+ / pwsh）
#
...
$PKG = '@zebbkira/dsh-skills-mcp-manager'
if ($Help) {
  Write-Host @'
@zebbkira/dsh-skills-mcp-manager 一键安装脚本（PowerShell）
...
用法：
  ...
  Source 缺省为当前包目录。示例：
    ...
    -Source @zebbkira/dsh-skills-mcp-manager   # 已发布到 npm/注册表时按名字装
'@
  exit 0
}
...
if ($DryRun) {
  ...
  Say '[dry-run] 然后校验 dsh.profile.bundles 包含 @zebbkira/dsh-skills-mcp-manager'
...
if ($bundles -notcontains $PKG) {
  Warn '@zebbkira/dsh-skills-mcp-manager 未出现在 dsh.profile.bundles 中——挂载未注册。'
